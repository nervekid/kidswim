...
<div class="ibox-title">
	...
	<form:form id="searchForm" modelAttribute="dict" action="${ctx}/sys/dict/" method="post" class="form-inline">
		...
		<div class="form-group">
			<span>标签 ：</span>
					<form:input path="label" htmlEscape="false"   onkeydown="keyDownEnter(event)"   maxlength="50" class="form-control input-sm"/>
			<span>类型：</span>
				<form:select id="type" path="type" class="form-control m-b"   onchange="search()" style="width:160px"><form:option value="" label=""/><form:options items="${typeList}" htmlEscape="false"/></form:select>
			<span>描述 ：</span>
				<form:input  placeholder="描述" path="description" htmlEscape="false"   onkeydown="keyDownEnter(event)"   maxlength="50" class="form-control input-sm"/>
		 </div>
	</form:form>
	<br/>
	</div>
	...
	<table id="contentTable" class="table table-striped table-bordered  table-hover table-condensed  dataTables-example dataTable no-footer">
		<thead>
			<tr>
				...
				<th  class="sort-column `label`">标签</th>
		...
		<c:forEach items="${page.list}" var="dict">
			<tr>
				...
				<td>${fns:getDictLabel(dict.status, 'data_status', '')}</td>
				...
				<td>
					...
					<shiro:hasPermission name="sys:dict:updateStatus">
						<c:if test="${dict.status eq '0'}">
							<a href="${ctx}/sys/dict/updateStatus?id=${dict.id}&type=${dict.type}&status=1" onclick="return confirmx('确认要停用该字典吗？', this.href)"   class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> 停用</a>
						</c:if>
						<c:if test="${dict.status eq '1'}">
							<a href="${ctx}/sys/dict/updateStatus?id=${dict.id}&type=${dict.type}&status=0" onclick="return confirmx('确认要启用该字典吗？', this.href)"   class="btn btn-success btn-xs"><i class="fa fa-trash"></i> 启用</a>
						</c:if>
					</shiro:hasPermission>
    				<%--<shiro:hasPermission name="sys:dict:del">
