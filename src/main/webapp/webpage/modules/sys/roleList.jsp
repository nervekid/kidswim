	...
	<form:form id="searchForm" modelAttribute="role" action="${ctx}/sys/role/" method="post" class="form-inline">
		...
		<div class="form-group">
			<span>角色名称：</span>
				...
				<span>系统模块：</span>
				<form:select path="sysModules"  onchange="search()"  class="form-control m-b">
		...
		<tbody>
			<c:forEach items="${list}" var="role">
				<tr>
					...
					<td>${fns:getDictLabel(role.sysModules, 'sysModules', '')}</td>
...
<%--						<c:if test="${(role.sysData eq fns:getDictValue('是', 'yes_no', '1') && fns:getUser().admin)||!(role.sysData eq fns:getDictValue('是', 'yes_no', '1'))}">
						...
						<a href="#" onclick="openDialogView('自定义权限', '${ctx}/sys/role/authorityindex?id=${role.id}','800px', '600px')"  class="btn  btn-warning btn-xs" ><i class="glyphicon glyphicon-plus"></i> 自定义权限</a>
