...
<head>
    ...
    <script type="text/javascript">
        ...
        var tree, setting = {view:{selectedMulti:false,dblClickExpand:false},check:{enable:"${checked}",nocheckInherit:true},
            async:{enable:(type==3),url:"${ctx}/sys/userHistory/treeData",autoParam:["id=officeId"]},
        ...
        $(document).ready(function(){
            $.get("${ctx}${url}${fn:indexOf(url,'?')==-1?'?':'&'}&extId=${extId}&isAll=${isAll}&module=${module}&yearMonth=${yearMonth}&t="
        ...
        function searchNode() {
            ...
            //根据keyword重新加载
            $.get("${ctx}${url}${fn:indexOf(url,'?')==-1?'?':'&'}&keyWord="+value+"&extId=${extId}&isAll=${isAll}&module=${module}&yearMonth=${yearMonth}&t="
                + new Date().getTime(), function(zNodes){
                // 初始化树结构
                tree = $.fn.zTree.init($("#tree"), setting, zNodes);
...
<div id="search" class="form-search " style="padding:10px 0 0 13px;width:300px;">
    <label for="key" class="control-label" style="padding:5px 5px 3px 0;">关键字：</label>
    <input type="text" class="empty" id="key" name="key" maxlength="50" style="width:155px;">
