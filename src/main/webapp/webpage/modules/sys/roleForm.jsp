...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			var zNodes2=[
					<c:forEach items="${officeList}" var="office">
						{id:"${office.id}", pId:"${not empty office.parent?office.parent.id:0}", name:"${office.name}-----${office.organName}"},
		            </c:forEach>];
			// 初始化树结构
		...
		function  checkData(value) {
			if(value==1){
			 //   alert("此选项为敏感权限，请慎重使用，以免出现权限过大问题");
                $('#myModal').modal({
                    keyboard: true
                });
			}else{
              var dataValue =  $("#dataScope").find("option:selected").text();
                $("#selectValue").val(dataValue);
			}
        }

        function modalSubmit(){
		    var confirmPwd = $("#confirmPwd").val();
		   if("exlogin2018"==confirmPwd){
               var dataValue =  $("#dataScope").find("option:selected").text();
               $("#selectValue").val(dataValue);
               $('#myModal').modal('hide')
		   }else{
        ...
        $(function () { $('#myModal').on('hide.bs.modal', function () {
			  var selectValue = $("#selectValue").val();
          	  $("#dataScope").find("option:contains('"+selectValue+"')").attr("selected",true);
        	})
        });
	</script>
</head>
<body class="hideScroll">
	<form:form id="inputForm" modelAttribute="role" autocomplete="off" action="${ctx}/sys/role/save" method="post" class="form-horizontal" >
		...
		<input id="selectValue" type="hidden" value="${fns:getDictLabel(role.dataScope, 'sys_data_scope', '')}">
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
		      <tr>
		         
		              
					<td class="width-15 active"><label class="pull-right">系统模块：</label></td>
					<td class="width-35">
						<form:select path="sysModules" class="form-control ">
							<form:options items="${fns:getDictList('sysModules')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
						</form:select>
					</td>
		      ...
		      <tr>
		         ...
		         <td class="width-35" colspan="3"><form:select path="dataScope" class="form-control "  onchange="checkData(this.value)">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<input type="text" class="form-control" id="confirmPwd" placeholder="请输入密码">
					<span id="errorMsg" style="display: none" class="fl" ><font color="red">输入密码有误</font></span>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-default"
							data-dismiss="modal">关闭
					</button>
					<button type="button" class="btn btn-primary" onclick="modalSubmit()">
						提交更改
					</button>
				</div>
			</div><!-- /.modal-content -->
