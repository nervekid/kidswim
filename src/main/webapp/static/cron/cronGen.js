(function ($) {
    ...
    $.fn.extend({
        cronGen: function (options) {
            ...
            $('<button id="popClose"  type="button" onclick="closeTab()" style="background-color: #fff;border: 0;position: absolute;margin-left: 15px;margin-top: -4px"><i class=\"glyphicon glyphicon-remove\"></i></button>').appendTo(topMenu);
            ...
            // Add an input
            var $i = $("<input>", { type: 'text',name:'jobCron', readonly:"readonly", id:"jobCron", placeholder: 'cron表达式...', style:'padding-left:16px; background:#fff'}).addClass("form-control cronIpt").val($(that).val());
            $i.appendTo($g);
            // Add the button
            var $b = $("<button class=\"btn btn-default btn-cron\" style='height: 40px'><i class=\"fa fa-edit\" style=\"color:#8d8d8d;text-shadow: 0 1px 0 #ffffff;\"></i></button>");
...
function closeTab(){
    $(".btn.btn-default.btn-cron").click();
}
(function($) {
