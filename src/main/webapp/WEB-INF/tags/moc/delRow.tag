...
<%@ attribute name="delLimitPoint" type="java.lang.String" required="false"%>
...
<script type="text/javascript">
	...
	function deleteAll(){
        ...
        if(${deleteLimitStatus == null}) {
            ...
            var delLimitPoint = '${delLimitPoint}';
            console.log("delLimitPoint__"+delLimitPoint);

            var arr = '';
            var flag = false ;
            if(delLimitPoint != '') {
                flag = true;
                arr = delLimitPoint.split(",");
            }
            ...
            for(var i=0;i<obj.length;i++) {
                ...
                for(var j=0; j<messageList.length; j++) {
                    var temp = messageList[j].split("_%_");
                    //console.log(temp);
                    ...
                    if(flag) {
                        if(targe.substring(arr[0],arr[1]) != value) {
                            top.layer.alert(msg, {icon: 0, title:'警告'});
                            return;
                        }
                    }else{
                        if(targe == value) {
