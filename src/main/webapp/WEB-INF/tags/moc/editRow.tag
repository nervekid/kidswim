...
<%@ attribute name="editLimitStatus" type="java.lang.String" required="false"%>
<%@ attribute name="editLimitPoint" type="java.lang.String" required="false"%>
...
$(document).ready(function() {
	...
	function edit(){
        ...
        if(status != '') {
            ...
            var inputStatus =  $("#${id} tbody tr td input.i-checks:checkbox:checked").attr(info[0]);
            console.log("inputStatus__"+inputStatus);

            var infoList = info[1].split(";")
            console.log("infoList__"+infoList);

            var editLimitPoint = '${editLimitPoint}';
            console.log("editLimitPoint__"+editLimitPoint);

            var subStr = "";
            if(editLimitPoint != '') {
                var arr = editLimitPoint.split(",");
                var subStr = inputStatus.substring(arr[0],arr[1]);
                console.log(subStr);
            }

            for(var i=0;i<infoList.length;i++) {
                var infoElement = infoList[i];
                var list = infoElement.split("_%_");



                if(subStr != '') {
                   if(subStr != list[0]) {
                        top.layer.alert(list[1], {icon: 0, title:'警告'});
                        return;
                    }
                }else {
                    if(inputStatus != list[0]) {
                        top.layer.alert(list[1], {icon: 0, title:'警告'});
                        return;
                    }
                }
