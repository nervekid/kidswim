...
<button class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="left" onclick="materialoutput()" title="设备领用"> ${label==null?'设备领用':label}</button>
                        </button>
	...
	function materialoutput(){
        ...
        var materialclass = $("#materialclass").val();
		  ...
		  if(types.indexOf("0")!=-1||types.indexOf("1")!=-1||types.indexOf("2")!=-1){
				top.layer.alert('已报废、在用、借出的设备不能领用！', {icon: 0, title:'警告'});
				return;
			  }
		...
		openDialog('${title}'+'领用',url,"${width==null?'800px':width}", "${height==null?'500px':height}","${target}");
