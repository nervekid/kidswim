...
import com.kite.common.security.PrincipalBase;
...
import com.kite.common.web.Servlets;
...
import com.kite.modules.sys.utils.LogUtils;
import com.kite.modules.sys.utils.UserUtils;
import com.kite.modules.sys.web.LoginController;
...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		String principalLoginName = (String) authcToken.getPrincipal();
			...
			if (sessions.size() > 0){
				for (Session session : sessions){
					...
					if(attribute!=null){
						...
						if(com.kite.common.utils.StringUtils.isNotEmpty(id)){
							User user1 = getSystemService().getUser(id);
							if(user1!=null && !StringUtils.equalsIgnoreCase("1",id)){
								String loginName = principalLoginName;
		...
		if (user != null) {
			...
			if(token.isScanCode()){ //扫描登录，自己模拟登录
				user.setPassword(token.getXuanwumw());
			}
	...
	public void initCredentialsMatcher() {
		...
		CustomerCredentialsMatcher matcher=new CustomerCredentialsMatcher();
...
//		}
	 ...
	 * 授权用户信息
	 */
	public static class Principal extends PrincipalBase  {

		public Principal(){

		}
		public Principal(User user, boolean mobileLogin) {
			super(user.getId(),user.getLoginName(),user.getName(),mobileLogin);
		}
