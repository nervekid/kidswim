...
public class SQLHelper {
    ...
    public static int getCount(final String sql, final Connection connection,
        ...
        String newSql =sql;
        if(sql.startsWith("select".toUpperCase())&&sql.indexOf("a.")!=-1 && sql.indexOf("FROM")!=-1){
            int begin = sql.indexOf("a.");
            int end = sql.indexOf("FROM");
            String headSQL =sql.substring(0,begin);
            String endSQL =sql.substring(end,sql.length());
            newSql = headSQL+" 1 "+endSQL;
        }
        final String countSql = "select count(1) from (" + newSql + ") tmp_count";
